...
export default function Home() {
  ...
  // Dipanggil dari TournamentPage saat player klik GO PLAY
  const handleTournamentPlay = (tournamentId: string) => {
    setActiveTournamentId(tournamentId)
    setTab('game')
  }

  // Dipanggil dari GameWrapper saat game over di tournament mode
      ...
      <div className="border-t border-farcaster/20 flex overflow-x-auto">
        {tabs.map(({ key, icon, label }) => (
          <button
            key={key}
            onClick={() => {
              // Reset tournament mode kalau user manually pindah tab game
